function [K_bar,K,A,B,N]=fh_consensus_constraim_1(eps)
...
for k = N-1:-1:1 
    ...
    if k == N-1   
        for i = 1:Step
            gamma(k,i) = x(1:2,i,k+1)'*100*Q* x(1:2,i,k+1) + x(1:2,i,k)' * Q * x(1:2,i,k) + x(3,i,k)' * R * x(3,i,k);  
        end
    else
        for i = 1:Step
    ...
    P_bar(:,:,k) = H_xx(:,:,k) + 4/20*K_bar(:,k)*H_ux(:,:,k)+H_xu(:,:,k)*4/20*K_bar(:,k)'+4/20*K_bar(:,k)*H_uu(k)*4/20*K_bar(:,k)';
...
for kk = N-1:-1:1
    if kk == N-1
        ...
        P(:,:,kk) = Q+A'*P(:,:,kk+1)*A - (2*4/(4*5)-(4/(4*5))^2)*A'*P(:,:,kk+1)*B*inv(R+B'*P(:,:,kk+1)*B)*B'*P(:,:,kk+1)*A;
    end
